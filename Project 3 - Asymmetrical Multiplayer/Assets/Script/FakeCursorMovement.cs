...
public class FakeCursorMovement : MonoBehaviour
{
    //GameObject mainCam;
    [HideInInspector]
    ...
    void MoveCursor()
    {
        if (canMove)
        {
            ...
            cursor.transform.position = new Vector3(icons[index].position.x, icons[index].position.y, 0); // place the cursor at its new position based on the icons position
